                    ...
                    try {
                        withContext(Dispatchers.IO) {
                            ...
                            if(response.isSuccessful && responseBody?.isNotEmpty() == true) {
                                ...
                                {
                                    throw CreaturesInsertException("Insert Passive Creatures failed : ${e.message}")
                                }
                            }else{
                                val errorCode = response.code()
                                val errorBody = response.errorBody()?.string() ?: "No error body"
                                throw CreaturesFetchLocalException("API Passive Creatures request failed with code $errorCode: $errorBody")
                            }
                        }
