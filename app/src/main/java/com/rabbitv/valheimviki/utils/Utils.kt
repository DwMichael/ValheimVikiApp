...
import com.rabbitv.valheimviki.presentation.detail.weapon.model.StatVisuals
...
fun mapUpgradeInfoToGridList(upgradeInfo: UpgradeInfo): List<GridLevelInfo> {
    ...
    upgradeInfo.upgradeLevels?.let { power ->
        val visual = StatVisuals.QUALITY
    ...
    upgradeInfo.fireDamage?.let { power ->
        val visual = StatVisuals.FIRE_DAMAGE
    ...
    upgradeInfo.frostDamage?.let { power ->
        val visual = StatVisuals.FROST_DAMAGE
    ...
    upgradeInfo.slashDamage?.let { power ->
        val visual = StatVisuals.SLASH_DAMAGE
    ...
    upgradeInfo.spiritDamage?.let { power ->
        val visual = StatVisuals.SPIRIT_DAMAGE
    ...
    upgradeInfo.durability?.let { power ->
        val visual = StatVisuals.DURABILITY
    ...
    upgradeInfo.stationLevel?.let { power ->
        val visual = StatVisuals.STATION_LEVEL
    ...
    upgradeInfo.lightningDamage?.let { power ->
        val visual = StatVisuals.LIGHTNING_DAMAGE
    ...
    upgradeInfo.pierceDamage?.let { power ->
        val visual = StatVisuals.PIERCE_DAMAGE
    ...
    upgradeInfo.bluntDamage?.let { power ->
        val visual = StatVisuals.BLUNT_DAMAGE
    ...
    upgradeInfo.poisonDamage?.let { power ->
        val visual = StatVisuals.POISON_DAMAGE
    ...
    upgradeInfo.pickaxeDamage?.let { power ->
        val visual = StatVisuals.PICKAXE_DAMAGE
    ...
    upgradeInfo.damageAbsorbedBloodMagic0?.let { power ->
        val visual = StatVisuals.DAMAGE_ABSORBED_BLOOD_MAGIC_0
    ...
    upgradeInfo.maximumSkeletonsControllable?.let { power ->
        val visual = StatVisuals.MAXIMUM_SKELETONS
    ...
    upgradeInfo.damageAbsorbedBloodMagic100?.let { power ->
        val visual = StatVisuals.DAMAGE_ABSORBED_BLOOD_MAGIC_100
    ...
    upgradeInfo.chopTreesDamage?.let { power ->
        val visual = StatVisuals.CHOP_TREES
