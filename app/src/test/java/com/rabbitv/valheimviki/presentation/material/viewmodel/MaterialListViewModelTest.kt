...
class MaterialListViewModelTest {
	...
	fun materialListFlow_NoCategorySelected_ShouldEmitEmptyList() = runTest {
		...
		viewModel.materialList.test {
			val result: List<Material> = awaitItem()
			assertEquals(emptyList(), result, "Should return empty list")
		}
	...
	@Test
	fun materialListFlow_CategoryChanged_ShouldEmitFilteredListByNewCategory() =
		runTest {
			val creatureDrop = List(4) { index ->
				Material(
					id = index.toString(),
			...
			viewModel.materialList.test {
				...
				val result2: List<Material> = awaitItem()
				assertEquals(
					miscList.sortedBy { it.order },
					result2,
					"Category is selected so full list should be emitted"
				)
				assertEquals(
	...
	@Test
	fun materialListFlow_CategorySelectedAndTypeSelected_ShouldEmitFilteredListByCategoryAndType() =
		runTest {

			val creatureDrops = listOf(
				Material(
					id = "1",
					name = "J Material Object Test",
					category = AppCategory.MATERIAL.toString(),
					subCategory = MaterialSubCategory.CREATURE_DROP.toString(),
			...
			val minibossList = listOf(
				Material(
					id = "4",
					name = "Material Object Test",
					category = AppCategory.MATERIAL.toString(),
					subCategory = MaterialSubCategory.MINI_BOSS_DROP.toString(),
					subType = MaterialSubType.LOOT.toString(),
					imageUrl = "",
					description = "",
					order = 4,
			...
			val materials = creatureDrops + miscList + minibossList
			...
			viewModel.materialList.test {
				...
				assertEquals(
					creatureDrops.filter { it.subType == MaterialSubType.LOOT.toString() }
						.sortedBy { it.name },
					result,
					"Category is selected so full list should be emitted"
				)
	...
	@Test
	fun materialListFlow_ChangedTypeToNull_ShouldEmitFilteredListOnlyByCategory() =
		runTest {
			...
			viewModel.materialList.test {
				...
				assertEquals(
					creatureDrops.filter { it.subType == MaterialSubType.LOOT.toString() }
						.sortedBy { it.name },
					result,
					"Category is selected so full list should be emitted result"
				)
				viewModel.onEvent(MaterialUiEvent.ChipSelected(null))
				val result2: List<Material> = awaitItem()

				assertEquals(
					creatureDrops.sortedBy { it.order },
					result2,
					"Category is selected so full list should be emitted result2"
				)
			}
		}

	@Test
	fun materialListFlow_MaterialTypeChanged_ShouldEmitFilteredListByNewType() =
		runTest {
			val creatureDrops = listOf(
				Material(
					id = "1",
					name = "H Material Object Test",
					category = AppCategory.MATERIAL.toString(),
					subCategory = MaterialSubCategory.CREATURE_DROP.toString(),
					subType = MaterialSubType.LOOT.toString(),
					imageUrl = "",
			...
			val materials = creatureDrops + miscList
			...
			viewModel.materialList.test {
				...
				assertEquals(
					creatureDrops.filter { it.subType == MaterialSubType.LOOT.toString() }
						.sortedBy { it.name },
					result,
					"Chip type is selected so full list should be emitted"
				)
				assertEquals(
					2,
					result.size,
					"Should be size of 2 type loot"
				)
				viewModel.onEvent(MaterialUiEvent.ChipSelected(MaterialSubType.TROPHY))
				val result2: List<Material> = awaitItem()
				assertEquals(
					creatureDrops.filter { it.subType == MaterialSubType.TROPHY.toString() }
						.sortedBy { it.name },
					result2,
					"Chip type is selected so full list should be emitted"
				)
				assertEquals(
					2,
					result2.size,
					"Should be size of 2 type trophy"
				)
			}
		}

	@Test
	fun materialListFlow_UpstreamError_ShouldBeCaughtAndCompletedWithError() = runTest {
