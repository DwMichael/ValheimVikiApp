...
class MiniBossesGridViewModelTest {
	...
	fun setUp() = runTest {
		...
		whenever(getMiniBossesUseCase()).thenReturn(flowOf(emptyList()))
	...
	@Test
	fun miniBossGridViewModel_uiState_initialLoading() = runTest {
		val viewModel = MiniBossesGridViewModel(creatureUseCases, connectivityObserver)

		viewModel.miniBossUiListState.test {
			assert(awaitItem() is UIState.Loading)
		}
	}

	@Test
	fun miniBossGridViewModel_uiState_emitsSuccessWhenDataIsAvailable() = runTest {
		val fakeMiniBossList: List<MiniBoss> = List(4) { index ->
			MiniBoss(
				id = "1",
				category = "MINIBOSS",
				subCategory = "",
		...
		whenever(getMiniBossesUseCase()).thenReturn(flowOf(fakeMiniBossList))
		val viewModel = MiniBossesGridViewModel(creatureUseCases, connectivityObserver)
		...
		viewModel.miniBossUiListState.test {
			...
			when (result) {
				...
				is UIState.Success -> {
					assertEquals(fakeMiniBossList, result.data)
				}
			}
	...
	@Test
	fun miniBossGridViewModel_uiState_emitsSuccessWhenDataIsAvailableWithNoConnection() = runTest {
	...
	@Test
	fun miniBossGridViewModel_uiState_emitsErrorWhenConnectionIsNotAvailableAndListIsEmpty() =
		runTest {
			whenever(getMiniBossesUseCase()).thenReturn(flowOf(emptyList()))
			whenever(connectivityObserver.isConnected).thenReturn(flowOf(false))
			val viewModel = MiniBossesGridViewModel(creatureUseCases, connectivityObserver)
	...
	@Test
	fun miniBossGridViewModel_uiState_remainsLoadingWhenConnectedAndListIsEmpty() = runTest {
